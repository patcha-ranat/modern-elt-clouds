...
class MongoLoader:
    ...
    def bulk_insert(self, data_path: str, db: str, collection: str, rows: int | bool = False):
        ...
        try:
            ...
            logging.info(f"Inserted {len(result.inserted_ids)} records to {db}.{collection}")
                ...
                if rows:
                    for i, line in enumerate(f):
                        if i+1 <= rows:
                            ...
                            logging.info(f"Inserted {result.inserted_id} to {db}.{collection}")
